...
class Register:
    ...
    def get_cw_data(self):
        ...
        for _ in range(self.retry):
            ...
            data = httpx.get(
                "{0}/api/v1/instances/{1}".format(self.endpoint, self.instance_id)
            )
    ...
    def check_script(self, candidate):
        self.log.debug("found the file: {0}".format(candidate))
        if not os.path.isfile(candidate):
            self.log.warning("{0} is not a file".format(candidate))
            return
    ...
    def run_scripts(self, script_type, failure_return_code=1):
        self.log.info("running {0} scripts".format(script_type))
        files = self.get_scripts(path="/etc/catweazle/{0}.d/".format(script_type))
        for _file in files:
        ...
        self.log.info("running {0} scripts, done".format(script_type))
