...
class CatWeazleLambda(object):
    def __init__(self, event, context):
        ...
        self._cw_role_session_name = os.environ.get(
    ...
    def instance_delete_v1(self):
        try:
            ...
            resp = self.catweazle_api(
                method="DELETE", path=path, body=None, api_version="v1"
            )
    ...
    def instance_delete_v2(self):
        try:
            ...
            path = f"/instances/{self.ec2_id}"
            resp = self.catweazle_api(
                method="DELETE", path=path, body=None, api_version="v2"
            )
            self.log.info(f"status: {resp.status_code}")
            if resp.status_code != 200:
                self.log.error(f"request error: {resp.text}")
    ...
    def instance_create_v1(self, body):
        try:
            ...
            path = f"/instances/{self.ec2_id}"
            resp = self.catweazle_api(
                method="POST", path=path, body=body, api_version="v1"
            )
            self.log.info(f"status: {resp.status_code}")
    ...
    def instance_create_v2(self, body):
        try:
            ...
            path = f"/instances/{self.ec2_id}"
            resp = self.catweazle_api(
                method="POST", path=path, body=body, api_version="v2"
            )
            self.log.info(f"status: {resp.status_code}")
            if resp.status_code != 201:
                self.log.error(f"request error: {resp.text}")
